...
import ese5934_project.bart.cfl as cfl
    ...
    class BartWrapper(tf2.Module):
        def __init__(self, model, dims, vars_as_input=True, name=None):
            ...
            if not (trace_complex):
                ...
                self.model(
                    np.zeros(self.dims_tf, np.float32)
                )  # run model ones to initialize weights
                ...
                self.model(
                    np.zeros(self.dims_tf, np.complex64)
                )  # run model ones to initialize weights
        ...
        def __call__(self, input, weights, grad_in):
            ...
            for i, input in enumerate(self.model.variables, 1):
                ...
                if self.vars_rtoc[i - 1]:
                    ...
                    result["grad_{}_0".format(i)] = tf2.stack(
                        [tmp, tf2.zeros_like(tmp)], axis=len(tmp.shape)
                    )
        ...
        def add_concrete_function(self, name=None):
            ...
            if self.trace_complex:
                signature_input = tf2.TensorSpec(
                    shape=dims, dtype=tf2.complex64, name="input_0"
                )
                signature_grad_ys = tf2.TensorSpec(
                    shape=dims, dtype=tf2.complex64, name="grad_ys_0"
                )
            else:
                signature_input = tf2.TensorSpec(
                    shape=dims, dtype=tf2.float32, name="input_0"
                )
                signature_grad_ys = tf2.TensorSpec(
                    shape=dims, dtype=tf2.float32, name="grad_ys_0"
                )

            signature_weight = []
            for i, var in enumerate(self.model.variables, 1):
                if self.vars_rtoc[i - 1]:
                    signature_weight.append(
                        tf2.TensorSpec(
            ...
            self.sig[name] = self.__call__.get_concrete_function(
                signature_input, signature_weight, signature_grad_ys
            )
                ...
                for signature in list(self.sig):
                    ...
                    for bart_name in list(inputs):
                        f.write(
                            "{} {} {}\n".format(
                                bart_name,
                                inputs[bart_name].name.split(":")[0],
                                inputs[bart_name].name.split(":")[1],
                            )
                        )
                    ...
                    for bart_name in list(outputs):
                        f.write(
                            "{} {} {}\n".format(
                                bart_name,
                                outputs[bart_name].name.split(":")[0],
                                outputs[bart_name].name.split(":")[1],
                            )
                        )
...
class TensorMap:
    ...
    def export(self):
        n = self.tensor.name
        return "{} {} {} {}".format(
            self.name, n.split(":")[0], n.split(":")[1], self.type
        )
...
def tf1_graph_attach_gradients(graph, inputs, outputs):
    ...
    for o, out in enumerate(outputs):
        with graph.as_default():
            gy = tf1.placeholder(
                out.tensor.dtype, shape=out.tensor.shape, name="grad_ys_" + str(o)
            )
    ...
    for i, inp in enumerate(inputs):
        for o, out in enumerate(outputs):
            ...
            with graph.as_default():
                ...
                grad = tf1.reshape(
                    grad, tf1.shape(inp.tensor), name="grad_{}_{}".format(i, o)
                )
